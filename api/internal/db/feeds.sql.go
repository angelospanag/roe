...
VALUES ($1, $2, $3, $4)
RETURNING id, title, url, description, link, favicon_url, last_fetched_at, created_at, updated_at
`
...
SELECT id, title, url, description, link, favicon_url, last_fetched_at, created_at, updated_at FROM feeds
WHERE id = $1
`
...
const updateFeedFavicon = `-- name: UpdateFeedFavicon :one
UPDATE feeds
...
func (q *Queries) UpdateFeedFavicon(ctx context.Context, arg UpdateFeedFaviconParams) (Feed, error) {
	row := q.db.QueryRow(ctx, updateFeedFavicon, arg.ID, arg.FaviconUrl)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.Title,
